...
export function FinanceClient({ initialBalanceInfo, initialTransactions, initialYear, initialMonth }: FinanceClientProps) {
  ...
  const TODAY = isMounted ? new Date().getDate() : new Date().getDate();
  ...
  const [balanceData, setBalanceData] = useState(initialBalanceInfo);
  ...
  useEffect(() => {
    // Only re-fetch if month/year changes and not initial render
